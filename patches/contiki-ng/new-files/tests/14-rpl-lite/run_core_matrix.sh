...
OUT_DIR="${PAPER_OUT_DIR:-$PAPER_14_DIR/core-results}"
LOG_DIR="$OUT_DIR/logs"
CSV="$OUT_DIR/core_matrix_results.csv"
...
load_existing_cases() {
  local group scene scale seed key
  local existing=0
  ...
  while IFS=, read -r group scene scale seed _; do
    if [ "$group" = "group" ]; then
      continue
    fi
    key="${group}|${scene}|${scale}|${seed}"
    DONE_CASES["$key"]=1
    existing=$((existing + 1))
  done < "$CSV"

  if [ "$existing" -gt 0 ]; then
    echo "resume: found $existing finished rows in $CSV"
  fi
}
...
main() {
  ...
  local scales=(20 40)
  local seeds
  local total idx group scene scale seed symbol key

  seeds=$(seq 1 20)
  total=$(( ${#groups[@]} * ${#scenes[@]} * ${#scales[@]} * 20 ))
